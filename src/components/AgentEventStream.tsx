...
const eventDisplayContent = (event: AgentEvent): string => {
          ...
          <div>
            ...
            {latestEvent && (
              <p className="mt-0.5 max-h-5 overflow-hidden text-xs leading-5 text-slate-400">
                最新：{eventLabels[latestEvent.kind]}{" "}
                {latestEvent.title
                  ? `· ${latestEvent.title}`
        ...
        <article className="grid grid-cols-[56px_22px_minmax(0,1fr)] gap-1.5" key={event.id}>
          ...
          <div className="relative flex justify-center">
            {index < visibleEvents.length - 1 && (
              <span className={`absolute top-6 bottom-[-0.25rem] w-px ${eventPriority[event.kind].line}`} />
            )}
            <span className={`relative z-10 grid h-4 w-4 place-items-center rounded-full border text-[9px] font-black ${eventPriority[event.kind].dot}`}>
              {eventPriority[event.kind].icon}
            </span>
          </div>
          <div
          ...
          >
            <div className="flex flex-wrap items-baseline gap-2">
              {event.kind === "thinking" || event.kind === "tool" ? (
                <span className="text-sm font-black text-slate-100">
                  {event.kind === "tool" ? eventLabels[event.kind] : event.title || eventLabels[event.kind]}
                </span>
              ) : null}
            ...
            {event.kind === "tool" && event.tool && <ToolEventDetails event={event} />}
