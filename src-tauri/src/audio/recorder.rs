...
use crate::vad::{VadEngine, VadEvent, VadStateMachine, HOP_SIZE, SAMPLE_RATE, THRESHOLD};
...
impl AudioRecorder {
    pub fn new(lib_path: &std::path::Path) -> Result<Self, RecorderError> {
        let vad_engine = VadEngine::new(lib_path, HOP_SIZE as i32, THRESHOLD)?;
