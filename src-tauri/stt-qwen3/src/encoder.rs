...
pub fn run_encoder(
    ...
    let outputs = sessions.encoder_conv
    ...
    let (conv_shape, conv_data) = conv_output.try_extract_tensor::<f32>().map_err(|e| SttError::InferenceError {
        model: "encoder_conv".into(),
        detail: format!("Failed to extract output: {}", e),
    })?;

    let conv_array = ndarray::Array3::from_shape_vec(
        (conv_shape[0] as usize, conv_shape[1] as usize, conv_shape[2] as usize),
        conv_data.to_vec(),
    ).map_err(|e| SttError::InferenceError {
        model: "encoder_conv".into(),
    ...
    let hidden_tensor = ndarray::Array2::from_shape_vec(
    ...
    let transformer_outputs = sessions.encoder_transformer
        .run(vec![
    ...
    let (output_shape, output_data) = transformer_output.try_extract_tensor::<f32>().map_err(|e| SttError::InferenceError {
        model: "encoder_transformer".into(),
        detail: format!("Failed to extract output: {}", e),
    })?;
