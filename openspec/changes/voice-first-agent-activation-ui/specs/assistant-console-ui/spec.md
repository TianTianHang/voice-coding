...
系统 SHALL 清晰展示监听、录音、转写、发送和连接状态，并在语音优先布局中突出当前阶段。
...
#### Scenario: 主舞台展示当前状态
- **WHEN** 用户处于任意语音交互阶段
- **THEN** 系统 SHALL 在主舞台区域展示当前阶段的状态文案
- **AND** 系统 SHALL 将该状态展示为页面第一视觉层级
...
### Requirement: 输出时间线默认按需展开
系统 SHALL 将完整输出时间线设为按需展开，并在关键交互场景自动提升可见性。

#### Scenario: 默认折叠时间线
- **WHEN** 系统进入常规待机或对话流程
- **THEN** 系统 SHALL 默认不占据主视图展示完整历史时间线
- **AND** 系统 SHALL 提供显式入口让用户展开完整时间线

#### Scenario: 确认或错误时自动展开
- **WHEN** 输出流出现 `confirm` 事件或 `error` 事件
- **THEN** 系统 SHALL 自动展开或提升时间线可见性
- **AND** 系统 SHALL 让用户无需额外搜索即可处理关键事件
