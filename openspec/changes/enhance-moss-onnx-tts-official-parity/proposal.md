...
- 增加 codec streaming decode 内部管线，允许合成过程中分批解码音频 chunk；本次仍保持现有“合成完成后播放”的外部播放契约。
...
- 质量验证包括 `nix develop -c cargo test`、`nix develop -c cargo clippy`、可选真实模型集成测试、`pnpm build` 与 `pnpm test`；完整桌面回归可执行 `nix develop -c pnpm tauri build`。
