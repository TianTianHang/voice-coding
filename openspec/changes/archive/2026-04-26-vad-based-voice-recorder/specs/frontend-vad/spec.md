...
The system SHALL continuously process audio frames through VAD to detect voice activity.

#### Scenario: Detect speech start
- **WHEN** VAD returns is_speech=1 for current frame
- **AND** current state is IDLE
...
The system SHALL display current VAD state and recording status to the user.

#### Scenario: Display listening status
- **WHEN** system is initialized and waiting for speech
- **THEN** system SHALL display "🎤 Listening..." indicator
- **AND** indicator SHALL use calming color (e.g., blue)
...
- **AND** system SHALL disable recording controls

#### Scenario: Display error status
- **WHEN** error occurs (VAD failure, microphone denied, etc.)
- **THEN** system SHALL display error message
- **AND** system SHALL provide guidance for resolution
